...
class Socks5Server:
    def __init__(
        ...
        username: str = os.getenv("PYTHON_SOCKS_SERVER_USERNAME", "user"),
        password: str = os.getenv("PYTHON_SOCKS_SERVER_PASSWORD", "password"),
    ):
        self.host = host
        self.port = port
    ...
    def handle_handshake(self, client_socket: socket.socket) -> bool:
        ...
        if 0x02 in methods:  # Username/password auth
            ...
            logger.info(
                f"Expected username: {self.username}, password: {self.password}"
            )
